...
Portanto, no contexto deste trabalho, o sistema de Visual SLAM alimenta o \textit{Navigation Stack} com a estimativa de pose e o mapa, permitindo que os algoritmos de planejamento descritos acima conduzam o robô de forma autônoma.
...
Além de seu gerenciamento de memória, o RTAB-Map possui módulos de construção de mapas 3D, navegação e integração com a pilha de navegação Nav2 (\textit{Navigation2}). Com esse conjunto, é possível converter o mapa 3D (proveniente da câmera RGB-D) em uma grade de ocupação 2D, permitindo que robôs equipados apenas com câmera executem os algoritmos de navegação padrão, sem a necessidade de um LiDAR (\textit{Light Detection and Ranging}) físico \cite{labbé2019rtabmapopensourcelidar}.
...
\section{Arquiteturas de Processamento em Robótica}

Para mitigar as restrições de hardware apresentadas, o projeto de um sistema robótico deve definir onde o processamento pesado (como o SLAM) será executado. Existem duas abordagens predominantes, cada uma com vantagens e desvantagens distintas que impactam a autonomia e a robustez do sistema.

O processamento local prioriza a independência do robô, eliminando a necessidade de infraestrutura externa, mas é limitado pela capacidade computacional embarcada. Por outro lado, o processamento remoto oferece recursos praticamente ilimitados, mas introduz uma dependência crítica da qualidade da conexão de rede \cite{li2021highefficientmultirobot}.

Considerando este compromisso (\textit{trade-off}), a decisão de projeto adotada neste trabalho consiste em implementar e avaliar ambas as estratégias. O objetivo é verificar experimentalmente se o ganho de desempenho obtido pelo descarregamento de processamento compensa os riscos de latência da rede em comparação à execução puramente embarcada no Raspberry Pi.

No processamento embarcado (\textit{Onboard Computing}), todos os módulos do SLAM (percepção, odometria e otimização) são executados localmente na CPU do robô. Esta abordagem garante autonomia total em relação à infraestrutura de rede, sendo robusta a falhas de conexão \cite{canovas2021onboarddynamicrgbd}.

No entanto, a abordagem \textit{onboard} em hardware de baixo custo exige sacrifícios. Para manter a operação em tempo real, muitas vezes é necessário reduzir a resolução das imagens, diminuir a frequência de atualização do mapa ou simplificar o algoritmo de extração de características, o que pode impactar a precisão final da navegação \cite{canovas2021onboarddynamicrgbd, costa2019characterizingpolybenchgpu}.

O processamento desembarcado, ou \textit{Computation Offloading}, consiste na estratégia de transferir as tarefas computacionalmente intensivas para um servidor remoto ou estação de trabalho (PC), mantendo no robô apenas as funções essenciais de aquisição de dados e atuação nos motores. Esta abordagem permite o uso de algoritmos de SLAM mais sofisticados e mapas mais densos, uma vez que o processamento ocorre em hardware de alto desempenho, poupando a bateria e a CPU do robô \cite{li2021highefficientmultirobot}.

Contudo, o \textit{offloading} introduz novos desafios, principalmente relacionados à rede sem fio. A transmissão de dados RGB-D exige alta largura de banda, e a instabilidade da conexão pode causar latência variável ou perda de pacotes. Conforme alertado por \citeonline{zheng2016multiframegraphmatching}, em cenários de baixa largura de banda, algoritmos de compressão e seleção de quadros tornam-se essenciais para evitar que o atraso na comunicação inviabilize o controle do robô em tempo real.

\section{Ferramentas de Desenvolvimento}

O desenvolvimento de sistemas robóticos complexos é facilitado por um ecossistema de software que permite a modularização, comunicação e simulação destes sistemas, conforme apresentado nesta seção.
...
O Gazebo é um simulador 3D robusto projetado para robótica, capaz de simular populações de robôs, sensores e objetos em um ambiente tridimensional. Ele integra uma \textit{Physics Engine} (como ODE ou Bullet) para simular dinâmica e colisões, e um motor de renderização (OGRE) para gerar dados visuais realistas para as câmeras do robô \cite{koenig2004designuseparadigms}.
...
A simulação no Gazebo é uma etapa útil para testar e validar a lógica de navegação e a integração entre componentes em um ambiente seguro e controlado, antes da integração e execução no protótipo físico. Contudo, a simulação não substitui a avaliação no hardware real, especialmente quanto a limitações de driver da câmera RGB-D, ruído real de sensores e desempenho de CPU/GPU em tempo real.