Este capítulo consolida os resultados obtidos nesta versão do protótipo físico do Talus-Droid durante a etapa de validação experimental. A análise apresentada teve escopo delimitado: verificar se a plataforma integrada foi capaz de adquirir dados RGB-D e inerciais, publicar transformações coerentes, acionar a base móvel e produzir odometria visual e estruturas iniciais de mapa em trajetórias curtas. Assim, os resultados fornecem evidências iniciais da viabilidade técnica da arquitetura e registram uma primeira linha de base de micro-mapeamento controlado, embora o SLAM autônomo em trajetórias longas permaneça como uma limitação tecnológica para a continuidade do trabalho.

\section{Linha de base experimental obtida}

A sequência recente de ensaios permitiu estabelecer uma linha de base operacional inicial para execução do RTAB-Map no robô físico. O pipeline mais estável observado até esta etapa utiliza os tópicos locais brutos do Kinect no Raspberry Pi, com imagem RGB, profundidade, informações de câmera e IMU processadas diretamente no robô. A configuração de referência emprega \texttt{approx\_sync\_max\_interval:=0.025}, iluminação ambiente ligada, inicialização da odometria visual com IMU habilitada por \texttt{wait\_imu\_to\_init:=true} e exclusão da odometria de rodas do pipeline de mapeamento. A bateria posterior de 21/05/2026 manteve essa leitura geral, acrescentando uma verificação de estabilidade parado, uma nova repetição de avanços curtos e uma calibração mínima de giros open-loop.

\begin{figure}[H]
    \centering
    \caption{Pipeline experimental de percepção RGB-D/RTAB-Map e atuação em malha aberta.}
    \includegraphics[width=0.95\textwidth]{figuras/talus_pipeline_ros_rtabmap_openloop.png}
    \label{fig:pipeline_rtabmap_openloop}
...
A Figura \ref{fig:deadband_motores_overview} complementa a tabela ao apresentar a contagem bruta dos sensores Hall nos ensaios de avanço linear e giro parado. Esses valores foram utilizados apenas como diagnóstico de movimento e simetria, não como odometria métrica oficial.

\begin{figure}[H]
    \centering
    \caption{Ensaio de banda morta para avanço linear e giro parado com contagem bruta dos sensores Hall.}
    \includegraphics[width=0.95\textwidth]{figuras/talus_motor_deadband_overview_v151_v155_v170_v175.png}
    \label{fig:deadband_motores_overview}
    \fonte{Elaboração própria (2026), a partir dos logs Hall v151-v155 e v170-v175.}
\end{figure}
...
A Tabela \ref{tab:vo-rtabmap-runs} apresenta runs representativos dessa evolução. Os resultados mais relevantes para a consolidação desta linha de base são v157 e v158, pois combinam movimento físico confirmado por ticks, configuração de atuação já ajustada e bancos RTAB-Map úteis, com baixa incidência de falhas de registro visual. Esses ensaios fornecem a principal evidência de micro-mapeamento controlado em trajetórias curtas.
...
As Figuras \ref{fig:vo_quality_summary} e \ref{fig:vo_trajectories_xy} sintetizam essa leitura. A primeira reúne proxies internos do RTAB-Map, como qualidade mediana da odometria visual, quantidade de nós no banco e falhas de registro. A segunda apresenta trajetórias XY relativas estimadas por odometria visual, normalizadas ao início de cada ensaio e sem referência externa de ground truth.

\begin{figure}[H]
    \centering
    \caption{Proxies internos de qualidade da odometria visual e tamanho do grafo RTAB-Map por ensaio.}
    \includegraphics[width=0.9\textwidth]{figuras/talus_vo_quality_summary.png}
    \label{fig:vo_quality_summary}
...
\hline
v166 & Robô parado, drift da VO & \texttt{quality\_median=278}, nenhuma falha de registro, banco com \texttt{Node=35} e \texttt{Feature=30929}, ticks \texttt{0/0} & Melhor evidência da rodada de baixo drift relativo após a inicialização. \\
...
Essa revisão reforça a leitura técnica preliminar alcançada durante os experimentos: o sistema não dispõe de odometria robusta para navegação 100\% autônoma, mas atinge microdeslocamentos calibráveis e uma odometria visual RGB-D funcional sob condições controladas. Para coletas futuras que deem continuidade ao projeto, o protocolo deverá incluir pré-aquecimento do Kinect, cena iluminada e texturizada, uso de tópicos raw locais, análise do drift apenas após a primeira odometria real e separação entre runs de qualidade de SLAM e runs de stress mecânico.

A Figura \ref{fig:cmd_odom_ticks_timeseries} aproxima os comandos aplicados, a resposta estimada por \texttt{/rtabmap/odom} e os ticks Hall em runs selecionados. A comparação reforça que os ticks foram úteis para confirmar atuação física, enquanto a odometria visual permaneceu como fonte principal de análise do movimento estimado.

\begin{figure}[H]
    \centering
    \caption{Séries temporais de comando, odometria visual e ticks Hall em ensaios selecionados.}
    \includegraphics[width=0.9\textwidth]{figuras/talus_cmd_odom_ticks_timeseries.png}
    \label{fig:cmd_odom_ticks_timeseries}
    \fonte{Elaboração própria (2026), a partir dos tópicos ROS 2 e logs Hall dos ensaios do Talus-Droid.}
\end{figure}
...
Como síntese desta etapa de integração, o Talus-Droid estabeleceu uma primeira linha de base experimental reproduzível para odometria visual RGB-D e micro-mapeamento em ambiente interno. Essa base tecnológica ainda convive com as restrições naturais do hardware de baixo custo, mas agrega valor direto à pesquisa ao registrar, na prática, as limitações físicas e operacionais de executar algoritmos complexos de SLAM sobre uma infraestrutura computacional restrita. O artefato passa a servir como plataforma validável para a continuidade das pesquisas em navegação robótica.
