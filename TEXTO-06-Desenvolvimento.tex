...
\section{Integração Alcançada}

O desenvolvimento do protótipo físico atingiu o objetivo de estabelecer uma plataforma funcional de robótica móvel voltada à pesquisa. A contribuição desta etapa é significativa: consolidou-se uma arquitetura de hardware de baixo custo com \textit{drivers} funcionais, comunicação entre componentes validada, sensores operacionais e integração suficiente para os experimentos de SLAM visual.

Esta base reduziu substancialmente os riscos técnicos de integração mecânica e eletrônica, habilitando a execução dos testes sistemáticos propostos na metodologia, cujos resultados, desempenho e limitações de mapeamento serão apresentados em detalhes no Capítulo \ref{cap-resultados-discussao}.
