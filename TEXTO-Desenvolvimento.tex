...
\begin{itemize}
    \item \textbf{SLAM Embarcado:} A execução estável de algoritmos de mapeamento visual no Raspberry Pi requer otimizações de software e ajustes finos de parâmetros. Os testes recentes já produziram micro-mapeamento controlado, mas ainda não validam SLAM robusto em trajetórias longas;
    ...
    \item \textbf{Evidências Experimentais:} A consolidação final das métricas de SLAM, incluindo perda de pose, estabilidade do mapa e comparação entre configurações, ainda depende da ampliação dos ensaios controlados e da exportação das visualizações dos melhores bancos RTAB-Map.
\end{itemize}
