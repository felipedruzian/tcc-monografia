Este capítulo detalha o estado atual de desenvolvimento do protótipo físico do robô de serviço proposto neste trabalho. A construção deste protótipo corresponde à materialização parcial do artefato definido na metodologia, especificamente na Etapa 2 da \textit{Design Science Research} (DSR), voltada ao desenvolvimento e modelagem da solução. Enquanto o protótipo simulado permanece em fase de concepção, o foco desta etapa concentrou-se em obter uma plataforma física funcional, capaz de integrar atuadores, sensores e comunicação em rede.

O propósito central do protótipo físico, na atual fase da pesquisa, é mitigar os riscos do projeto ao assegurar uma base realista e operacional para os experimentos previstos na continuação do trabalho. O artefato deve viabilizar, em etapa posterior, a comparação entre diferentes algoritmos e configurações de sensores, bem como a confrontação entre os resultados obtidos no hardware real e no ambiente simulado. Assim, ainda que um sistema completo de navegação autônoma não tenha sido alcançado nesta fase, consolidou-se uma infraestrutura de hardware e software suficiente para a realização dos testes planejados.
...
\begin{itemize}
    ...
    \item Viabilizar a leitura contínua dos dados da unidade de medição inercial (IMU) e estabelecer o fluxo de publicação desses dados no ambiente \textit{Robot Operating System} (ROS), incluindo a transmissão pela rede local para uma estação de trabalho;
    \item Realizar testes preliminares de integração da câmera RGB-D com o ROS, preparando a infraestrutura para a futura execução de algoritmos de vSLAM;
    \item Comprovar a viabilidade técnica da arquitetura híbrida (embarcada/desembarcada), na qual parte do processamento pode ser transferida para um computador externo conectado à mesma rede local.
\end{itemize}

Dessa forma, ainda que um pipeline completo de SLAM em tempo real e a navegação autônoma não tenham sido efetivamente implementados, consolidou-se a infraestrutura necessária para que essas etapas possam ser abordadas com menor incerteza na continuidade do projeto.
...
A organização do sistema segue uma abordagem em camadas. Na base, encontra-se o chassi mecânico com o sistema de tração e a fonte de energia primária. Acima, a camada de hardware embarcado gerencia os sinais elétricos, a leitura de sensores e a comunicação serial. Na camada superior, a unidade de processamento executa o sistema operacional e o \textit{middleware} robótico, centralizando a lógica de navegação e a interface de rede.
...
\begin{itemize}
    ...
    \item \textbf{Infraestrutura de Comunicação:} Configuração do \textit{middleware} Eclipse Cyclone DDS \cite{eclipse2021eclipsecyclonedds} para garantir a transmissão eficiente de dados de alta largura de banda, como imagens e mapas de profundidade, através da rede local;
    \item \textbf{Pré-processamento de Navegação:} Nós preliminares de visualização e SLAM, configurados para receber os fluxos de dados da câmera e da IMU, preparando o sistema para a execução dos algoritmos de mapeamento.
\end{itemize}
...
Com a integração física e lógica concluída, foram realizados experimentos funcionais para validar a operabilidade do protótipo. Estes testes tiveram caráter exploratório, visando assegurar que a infraestrutura de \textit{hardware}, a pilha de \textit{software} e a comunicação em rede atendem aos requisitos mínimos para os experimentos de navegação previstos na próxima fase da pesquisa.

\subsection{Validação da Arquitetura Distribuída}

A arquitetura proposta fundamenta-se na capacidade de distribuir o processamento entre o robô embarcado e uma estação de trabalho externa (\textit{offboard}).  Para validar este requisito, o protótipo foi conectado à rede local via Wi-Fi (5 GHz), recebendo endereçamento estático para facilitar a descoberta de nós pelo \textit{middleware} Cyclone DDS. 

A estabilidade da transmissão de dados foi verificada monitorando a frequência de recebimento na estação remota. A Figura \ref{fig:hz_remoto} exibe o terminal da estação de trabalho recebendo o tópico de profundidade a uma taxa média de 30.04 Hz, idêntica à taxa de geração no robô. Isso confirma que a rede local suporta o tráfego intenso sem perda significativa de pacotes, validando a viabilidade técnica da estratégia de \textit{offloading} para o processamento pesado de SLAM.
...
Além da frequência, analisou-se a largura de banda exigida para a transmissão dos dados visuais, um fator crítico em arquiteturas distribuídas. Conforme apresentado na Figura \ref{fig:bandwidth_rede}, a transmissão das imagens brutas (\textit{raw}) de profundidade e RGB consome, respectivamente, cerca de 5.55 MB/s e 6.75 MB/s. Este alto volume de dados justifica a escolha da frequência de 5 GHz para a rede sem fio e evidencia a importância de estratégias de compressão de imagem (tópicos \texttt{/compressed}) caso o ambiente apresente instabilidade de sinal.
...
\begin{itemize}
    ...
    \item \textbf{Autonomia de Navegação:} A pilha de navegação (\textit{Nav2}) para planejamento de trajetória e desvio de obstáculos ainda não foi integrada ao controle de baixo nível;
    \item \textbf{Simulação:} A modelagem do ``gêmeo digital'' no simulador Gazebo permanece em fase de concepção e será priorizada na próxima etapa para permitir a validação cruzada dos experimentos.
\end{itemize}

Apesar destas lacunas, a contribuição desta etapa é significativa: consolidou-se uma plataforma de baixo custo com \textit{drivers} funcionais, comunicação de rede estável e integração de hardware validada. Esta base sólida reduz substancialmente o risco técnico do projeto e habilita a execução dos experimentos sistemáticos de desempenho de vSLAM propostos na metodologia.