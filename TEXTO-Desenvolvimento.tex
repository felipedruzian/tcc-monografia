...
Um dos principais desafios do uso de vSLAM em SBCs (\textit{Single Board Computers}) é a sobrecarga da CPU e do barramento USB durante a aquisição de imagens. Conforme demonstra a Figura \ref{fig:hz_local}, em testes isolados de aquisição, o sistema atingiu a taxa nominal de aproximadamente 30.03 Hz. No entanto, a manutenção dessa taxa de forma sincronizada para fluxos simultâneos de RGB e profundidade revelou-se instável durante a execução conjunta de outros nós do sistema, evidenciando a necessidade de otimização no transporte de dados para evitar perdas de quadros que degradem a qualidade do SLAM.
...
\begin{itemize}
    \item \textbf{Estabilidade do Sensor RGB-D:} A aquisição simultânea de dados de imagem e profundidade no Kinect v1 via USB 2.0 do Raspberry Pi apresenta instabilidades aleatórias de sincronização e queda de taxa de quadros, constituindo um dos principais gargalos técnicos do projeto;
    \item \textbf{SLAM Embarcado:} A execução estável de algoritmos de mapeamento visual no Raspberry Pi requer otimizações rigorosas para evitar a saturação da CPU, o que impacta diretamente na frequência de atualização do mapa;
